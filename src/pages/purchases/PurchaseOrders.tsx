...
const statusLabels: Record<string, string> = { draft: "مسودة", confirmed: "مؤكد", partially_received: "مستلم جزئياً", received: "مستلم", cancelled: "ملغي", closed: "مغلق" };
...
export default function PurchaseOrders() {
  ...
  const [form, setForm] = useState({ order_date: new Date().toISOString().slice(0, 10), expected_date: "", supplier_id: "", warehouse_id: "", notes: "" });
  const [lines, setLines] = useState<any[]>([{ product_id: "", quantity: 1, unit_price: 0, tax_id: null, tax_percent: 0, line_total: 0 }]);

  const load = async () => {
    const [r, s, p, w, t] = await Promise.all([
      supabase.from("purchase_orders").select("*, suppliers(name)").order("order_date", { ascending: false }),
      supabase.from("suppliers").select("id,code,name").eq("is_active", true),
    ...
    setRows(r.data || []); setSuppliers(s.data || []); setProducts(p.data || []); setWarehouses(w.data || []); setTaxes(t.data || []);
  };
  useEffect(() => { load(); }, []);
  ...
  const save = async () => {
    if (!activeBranch || !form.supplier_id) return toast.error("بيانات ناقصة");
    const order_number = `PO-${activeBranch.code}-${Date.now().toString().slice(-6)}`;
    const { data: h, error } = await supabase.from("purchase_orders").insert({
      order_number, order_date: form.order_date, expected_date: form.expected_date || null,
      supplier_id: form.supplier_id, branch_id: activeBranch.id, warehouse_id: form.warehouse_id || null,
      subtotal, tax_amount: taxAmount, total_amount: total, notes: form.notes, status: "draft", created_by: user!.id,
    }).select().single();
    if (error) return toast.error(error.message);
    await supabase.from("purchase_order_lines").insert(lines.map(l => ({ order_id: h.id, ...l })));
  ...
  const confirm = async (id: string) => {
    await supabase.from("purchase_orders").update({ status: "confirmed", confirmed_by: user!.id, confirmed_at: new Date().toISOString() }).eq("id", id);
    toast.success("تم تأكيد الأمر"); load();
  };

  const filtered = rows.filter(r => !search || r.order_number?.includes(search) || r.suppliers?.name?.includes(search));

  return (
    <div className="space-y-4">
      <ListPageHeader title="أوامر الشراء" breadcrumbs={[{ label: "الرئيسية", href: "/" }, { label: "المشتريات" }, { label: "أوامر الشراء" }]} searchValue={search} onSearchChange={setSearch} onAdd={() => setOpen(true)} onRefresh={load} />
      <div className="bg-card border rounded-lg">
        <Table>
          <TableHeader><TableRow><TableHead>الرقم</TableHead><TableHead>التاريخ</TableHead><TableHead>المورد</TableHead><TableHead>الإجمالي</TableHead><TableHead>الحالة</TableHead><TableHead></TableHead></TableRow></TableHeader>
          <TableBody>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div><Label>التاريخ المتوقع</Label><Input type="date" value={form.expected_date} onChange={e => setForm({ ...form, expected_date: e.target.value })} /></div>
          ...
          <div className="border rounded-lg mt-3">
            ...
            <Table>
              ...
              <TableBody>
                {lines.map((l, i) => (
                  <TableRow key={i}>
                    <TableCell><Select value={l.product_id} onValueChange={v => { const p = products.find(x => x.id === v); updateLine(i, { product_id: v, unit_price: Number(p?.cost_price) || 0 }); }}><SelectTrigger><SelectValue placeholder="اختر" /></SelectTrigger><SelectContent>{products.map(p => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent></Select></TableCell>

...
export default function SalaryComponents() {
  ...
  const empty = { code: "", name: "", component_type: "earning", calc_method: "fixed", default_value: 0, is_taxable: false, is_gosi_subject: false, is_active: true, sort_order: 0 };
  const [form, setForm] = useState<any>(empty);

  const fetchData = async () => {
    setLoading(true);
    const c = await supabase.from("hr_salary_components").select("*").order("sort_order");
  ...
  const save = async () => {
    ...
    const payload: any = { ...form, default_value: Number(form.default_value), sort_order: Number(form.sort_order) };
    ...
    <div>
      ...
      <div className="bg-card border border-t-0 rounded-b-lg p-4">
        {loading ? <Loader2 className="animate-spin mx-auto" /> : (
          <Table>
            <TableHeader><TableRow><TableHead>الكود</TableHead><TableHead>الاسم</TableHead><TableHead>النوع</TableHead><TableHead>طريقة الحساب</TableHead><TableHead>القيمة</TableHead><TableHead>نشط</TableHead><TableHead></TableHead></TableRow></TableHeader>
            <TableBody>
              {data.map(r => (
                <TableRow key={r.id}>

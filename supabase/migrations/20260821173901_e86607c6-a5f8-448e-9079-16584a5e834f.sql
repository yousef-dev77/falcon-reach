ALTER TABLE public.screen_permissions
  ADD COLUMN IF NOT EXISTS deny_view boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS deny_create boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS deny_edit boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS deny_delete boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS deny_approve boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS deny_post boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS deny_export boolean NOT NULL DEFAULT false;

DROP TABLE IF EXISTS public.resource_relations CASCADE;
...
AS $$
  SELECT COALESCE((
    SELECT CASE _action
      ...
      ELSE false
    END
    FROM public.get_effective_screen_permissions(_user_id) p
    WHERE p.screen_code = _screen_code
  ), false);
$$;
...
GRANT EXECUTE ON FUNCTION public.get_effective_screen_permissions(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_effective_screen_permissions(uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.has_screen_access(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.has_screen_access(uuid, text, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.explain_screen_access(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.explain_screen_access(uuid, text, text) TO service_role;